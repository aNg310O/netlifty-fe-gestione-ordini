...
const AdminMonthOrder = () => {
      ...
      const getData = async (str) => {
        try {
            ...
            if (res.data.length !== 0) {
                ...
                setLoading(false)
                setEmpty(false);
                console.log(`INFO, ${seller.username}, ordini.table.monthOrder.component, getData monthyearReport`)
            } else {
                setEmpty(true);
                setMsg(`${seller.username}, non ci sono ordini per la data selezionata`);
                console.log(`INFO, ${seller.username}, ordini.table.monthOrder.component, getData month order yet`)
                    }
        } 
        catch (e) {
            if (e.message === "Network Error") {
                ...
                setResult("Sessione scaduta. Fai logout/login!")
                setOpen(true);
                      Logging.log("ERROR", seller.username, "ordini.table.monthOrder.component", `getData errore ${e.message}`)
                console.log(`ERROR , ${seller.username}, ordini.table.monthOrder.component, getData errore ${e.message}`)
                    } else if (e.response.status === 403) {
                setLoading(false);
                      setSnackColor('red');
                      setResult("No token provided. Fai logout/login!")
    ...
    const handleReportClick = () => {
        var test = new jsPDF();
        test.text(`Report ordini per il mese ${fileName.substring(15,19)} - ${fileName.substring(19,21)}`, 10, 15);
        ...
        if(isChrome || isFirefox || isSafari || isOpera || isIE || isEdge || isYandex || isChromium || isMobileSafari){
            ...
            console.log(`INFO, ${seller.username}, ordini.table.monthOrder.component, handleReportClick download from browser`)
            Logging.log("INFO", seller.username, "ordini.table.monthOrder.component", `handleReportClick download from browser`) 
        } else {
            ...
            console.log(`INFO, ${seller.username}, ordini.table.monthOrder.component, handleReportClick download from mobile`)
            Logging.log("INFO", seller.username, "ordini.table.monthOrder.component", `handleReportClick download from mobile`) 
        }
    };
