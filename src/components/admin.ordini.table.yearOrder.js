...
const AdminYearOrder = () => {
    ...
    const retrieveMonth = () => {
        API.get('gestione-ordine/availableYear', { headers: authHeader() })
          .then(response => {
            if (response.status === 200) {
              setTests(response.data);
              setLoading(false);
              console.log(`INFO, ${currentUser.username}, ordini.table.yearOrder.component, retrieveYear() Call OK`)
            }
          })
          .catch(e => {
            if (e.message === "Network Error") {
              ...
              setOpen(true);
              Logging.log("ERROR", currentUser.username, "ordini.table.yearOrder.component", `retrieveYear() Call KO ${e.message}`)
              console.log(`ERROR, ${currentUser.username}, ordini.table.yearOrder.component, retrieveYear() Call KO ${e.message}`)
